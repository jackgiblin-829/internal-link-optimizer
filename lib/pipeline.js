...
import { getArticle } from "./fetchArticle.js";
 ...
 */
export async function runPipeline({ article, domain, maxLinks }, emit) {
  const step = (n, title) => ({
  ...
  s1.start(looksUrl(article) ? `Fetching ${article}` : "Using provided content");
  ...
  const wide = await discoverCandidates(domain, searchTerms, { perTerm: 8, cap: 45, exclude });
  ...
  const titles = await fetchTitles(wide.candidates, { concurrency: 8 });
  ...
  const dupSig = (u) =>
    safeDecode(u)
      ...
      .split(/[/?#]/)
      .filter(Boolean)
      .map((seg) => seg.split("-").filter(Boolean).sort().join("-"))
      .join("/");
  ...
  // 5. Match verbatim anchors to the best candidate page (anchor,url pairs).
  ...
  // 7. Reverse suggestions: existing pages that should link TO this article.
  ...
  const proposed = edits.map((e) => {
    const a = application.added.find((x) => x.url === e.url && x.anchor === e.anchor);
    return {
      anchor: e.anchor,
      url: e.url,
...
function looksUrl(s) {
  return /^https?:\/\/\S+$/i.test((s || "").trim().split("\n")[0]);
}

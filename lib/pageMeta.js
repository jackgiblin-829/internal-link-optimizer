...
async function fetchTitle(url, timeoutMs = 6000) {
  try {
    const res = await safeFetch(url, {
      ...
      timeoutMs,
      maxBytes: 512 * 1024, // the <title> lives in <head>; no need to pull the whole page
    });
    if (!res.ok) return null;
    const m = res.text.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
    if (!m) return null;
  ...
  async function worker() {
    while (i < urls.length) {
      ...
      if (!promise) {
        ...
        // A failed fetch resolves to null (never rejects), so without this a
        // single transient blip on one article would permanently deny every
        // later article in the same batch a title for this URL. Let the next
        // article that needs it retry fresh instead of reusing the failure.
        promise.then((title) => {
          if (title === null && cache?.get(url) === promise) cache.delete(url);
        });
      }
